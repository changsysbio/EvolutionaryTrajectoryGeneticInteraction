function [classification] = MutationClassificationBeneficialPositiveEpistasis(classification, cf)
	...
	assert(1 < cf.fitness_b);
	assert(cf.fitness_b < cf.fitness_ab);

    mutationRateThreshold = 2e-6;
    thresholdForHighMutationRate = 0.05;
    thresholdForLowMutationRate = 0.1;
    ...
    if ( cf.mutation_rate_b > mutationRateThreshold & ...
        ...
        if ( strcmp(fitnessType, 'beneficial'))
            classification.type = 'beneficial; with positive epistasis';
        else
            classification.type = 'neutral to beneficial';
        end            
        return;
